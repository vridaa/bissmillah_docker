...
async function fetchNotes() {
    ...
    let notesHtml = "";
    data.forEach(note => {
        let categoryClass = ["tugas", "hutang", "kerja", "penting"].includes(note.category) ? note.category : "lainnya";
...
async function deleteNote(id) {
    if (confirm("Yakin ingin menghapus catatan?")) {
        await fetch(`http://localhost:4000/delete-notes/${id}`, { method: "DELETE" });
        fetchNotes();
    }
}
